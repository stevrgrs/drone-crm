...
export default function SearchResultsClient({ initialCards }: any) {
  ...
  async function updateCustomer(id: any, patch: any) {
    ...
    setCards((c: any[]) => c.map((x: any) => (x.id === id ? { ...x, ...patch } : x)))
  }
  ...
  async function updateJob(id: any, patch: any) {
    ...
    setCards((c: any[]) =>
      c.map((card: any) => ({
        ...card,
        jobs: card.jobs.map((j: any) => (j.id === id ? { ...j, ...patch } : j)),
      }))
    )
  }
  ...
  async function addRepair(customerId: any) {
    ...
    setCards((c: any[]) => c.map((x: any) => (x.id === customerId ? { ...x, jobs: [data, ...x.jobs] } : x)))
  }
  ...
  async function deleteRepair(id: any) {
    ...
    setCards((c: any[]) =>
      c.map((card: any) => ({
        ...card,
        jobs: card.jobs.filter((j: any) => j.id !== id),
      }))
    )
  }
  ...
  async function deleteCustomer(id: any) {
    ...
    setCards((c: any[]) => c.filter((x: any) => x.id !== id))
  }
      ...
      {cards.map((customer: any) => (
        <div key={customer.id} className="rounded-2xl border border-slate-800 bg-[#0b1220] p-5">
          ...
          <div className="mt-4 flex justify-between items-center">
            <h3>Repairs</h3>
            <button onClick={() => addRepair(customer.id)} className="bg-red-600 px-3 py-1 rounded">
              Add Repair
            </button>
          </div>
            ...
            {customer.jobs.map((job: any) => (
              <div key={job.id} className="border p-3 rounded">
                ...
                <div className="flex gap-2 mt-2">
                  <Link href={`/jobs/${job.id}/photos`} className="border px-3 py-1 rounded">
                    Photos
                  </Link>
                  <button onClick={() => deleteRepair(job.id)} className="bg-red-700 px-3 py-1 rounded">
                    Delete
                  </button>
                </div>
              </div>
          ...
          <div className="flex justify-end mt-4">
            <button onClick={() => deleteCustomer(customer.id)} className="bg-red-800 px-4 py-2 rounded">
              Delete Customer
            </button>
          </div>
        </div>
